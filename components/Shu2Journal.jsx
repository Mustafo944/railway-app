...
export default function Shu2Journal({ station, workerName, mode, onClose }) {
  ...
  useEffect(() => { if (mode === 'archive') loadData(); }, [mode]);

  const loadData = async () => {
    const { data } = await supabase.from('shu2_journal').select('*').eq('station', station).order('created_at', { ascending: false });
    if (data) {
      setList(data);
      const grouped = {};
      data.forEach(r => {
  ...
  const save = async () => {
    ...
    setIsSaving(true);
    const { error } = await supabase.from('shu2_journal').insert({ station, worker_name: workerName, ...form });
    if (!error) { toast.success("SHU-2 ga yozildi!"); setForm(emptyForm); onClose(); }
        ...
        <div className="bg-white w-full max-w-3xl rounded-3xl overflow-hidden flex flex-col max-h-[95vh]">
          <div className="flex justify-between items-center px-5 py-4 border-b bg-green-50 shrink-0">
            <div className="flex items-center gap-3">
              {step !== 'months' && (
                <button onClick={handleBack} className="text-green-900 font-black text-xs flex items-center gap-1 cursor-pointer hover:underline">
                  <ArrowLeft size={16}/> {step === 'days' ? 'Oylar' : 'Kunlar'}
                </button>
              )}
              <h2 className="text-base font-black text-green-900 uppercase">
                {step === 'months' && `📒 SHU-2 Arxiv — ${station}`}
            ...
            <button onClick={onClose} className="bg-slate-100 p-2 rounded-full cursor-pointer hover:bg-slate-200"><X size={20}/></button>
          </div>
    ...
    <div className="fixed inset-0 bg-black/80 z-210 flex items-center justify-center p-2 sm:p-4">
      <div className="bg-white w-full max-w-md rounded-3xl overflow-hidden">
        <div className="flex justify-between items-center px-6 py-4 border-b bg-green-50">
          ...
          <button onClick={onClose} className="bg-white p-2 rounded-full cursor-pointer hover:bg-slate-100"><X size={20}/></button>
        </div>
        <div className="p-6 space-y-4">
          <div>
            <label className="text-[10px] font-black text-slate-500 uppercase">Sana</label>
            <input value={form.sana} onChange={e => setForm({ ...form, sana: e.target.value })} placeholder="11.03.2026"
              className="w-full p-3 border-2 rounded-xl outline-none focus:border-green-600 font-bold text-sm mt-1"/>
          </div>
          <div>
            <label className="text-[10px] font-black text-slate-500 uppercase">Navbatchilikdagi yozuv va bajarilgan ishlar nomi</label>
            <textarea value={form.ish_nomi} onChange={e => setForm({ ...form, ish_nomi: e.target.value })}
              placeholder="Bajarilgan ish yoki yozuvni kiriting..." rows={5}
              className="w-full p-3 border-2 rounded-xl outline-none focus:border-green-600 font-bold text-sm mt-1"/>
          </div>
