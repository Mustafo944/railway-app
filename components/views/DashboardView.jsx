// railway-app/components/views/DashboardView.jsx
import { ArrowLeft, Clock, User, CheckCircle, AlertTriangle, Loader2 } from 'lucide-react';
import { formatFullDateTime } from '@/lib/utils';

export default function DashboardView({
  selectedStation, activeTasks, activeFaults,
  currentWorker, isLoadingTasks,
  onBack, onShowFaultModal, onShowArchive,
  onShowStationFaultArchive, onShowTaskMenu,
  onSetConfirmFinishTask, onResolveFault
}) {
  const currentFault = activeFaults.find(f => f.station === selectedStation && f.status === "active");
...
<span className="text-[10px] font-bold text-slate-500">
  👤 {currentFault.worker_name || "Noma'lum"}
            ...
            {currentWorker?.role === 'worker' && (
              <button
                onClick={() => onResolveFault(currentFault)}
        ...
        <div className="fixed bottom-0 left-0 right-0 sm:static flex gap-1 font-black text-[9px] uppercase p-3 bg-white/95 backdrop-blur sm:p-0 sm:bg-transparent sm:ml-auto shadow-[0_-4px_20px_rgba(0,0,0,0.08)] sm:shadow-none">
          ...
          <button
            onClick={onShowStationFaultArchive}
        ...
        {isLoadingTasks ? (
          ...
          activeTasks.map((task, index) => (
            <div
              ...
              className="bg-white p-6 rounded-4xl shadow-xl border-l-8 border-l-orange-500 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 animate-in slide-in-from-left duration-300"
            >
              ...
              <button
                onClick={() => onSetConfirmFinishTask(task.id)}
