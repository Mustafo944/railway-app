...
import { X, ArrowLeft, Plus, Download } from 'lucide-react';

const BASE_URL = 'https://shchbukhara.netlify.app';
...
export default function TaskMenu({
  ...
  const openPdf = (type) => setPdfModal(type);
  const closePdf = () => setPdfModal(null);

  const getPdfUrl = (type) => type === 'haftalik' ? '/jadval-haftalik.pdf' : '/jadval-yillik.pdf';
  const getGoogleUrl = (type) => `https://docs.google.com/viewer?url=${encodeURIComponent(`${BASE_URL}${getPdfUrl(type)}`)}&embedded=true`;
            ...
            {taskMenuStep === 'main' && (
              <div className="flex flex-col gap-4">
                ...
                <div className="border-2 border-slate-100 rounded-3xl overflow-hidden">
                  <p className="text-[10px] font-black text-slate-400 uppercase px-5 pt-4 pb-2">📄 Grafiklarni ko'rish</p>
                  <div className="flex gap-3 px-4 pb-4">
                    <button
                      onClick={() => openPdf('haftalik')}
                      className="flex-1 p-4 rounded-2xl bg-green-50 border-2 border-green-100 hover:bg-green-700 hover:text-white hover:border-green-700 transition-all cursor-pointer group"
                    >
                      <p className="text-2xl mb-1">📅</p>
                      <p className="font-black text-xs text-green-800 group-hover:text-white">4 haftalik</p>
                      <p className="text-[9px] text-green-600 group-hover:text-green-100 font-bold mt-0.5">Ko'rish & Yuklab olish</p>
                    </button>
                    <button
                      onClick={() => openPdf('yillik')}
                      className="flex-1 p-4 rounded-2xl bg-purple-50 border-2 border-purple-100 hover:bg-purple-700 hover:text-white hover:border-purple-700 transition-all cursor-pointer group"
                    >
                      <p className="text-2xl mb-1">📋</p>
                      <p className="font-black text-xs text-purple-800 group-hover:text-white">Yillik</p>
                      <p className="text-[9px] text-purple-600 group-hover:text-purple-100 font-bold mt-0.5">Ko'rish & Yuklab olish</p>
                    </button>
                  </div>
                </div>
              </div>
        ...
        <div className="fixed inset-0 z-[60] flex flex-col bg-black">
          ...
          <div className="flex items-center justify-between px-4 py-3 bg-slate-900 shrink-0">
            <h3 className="font-black text-white text-sm uppercase">
              {pdfModal === 'haftalik' ? '📅 4 Haftalik reja' : '📋 Yillik reja'}
            </h3>
            <button onClick={closePdf} className="bg-red-600 hover:bg-red-500 text-white p-2 rounded-xl cursor-pointer transition">
              <X size={16}/>
            </button>
          </div>
          ...
          {/* PASTKI — YUKLAB OLISH */}
          <div className="bg-slate-900 px-4 py-3 shrink-0 flex justify-center gap-3">
            <a
              href={getPdfUrl(pdfModal)}
              download
              className="bg-green-600 hover:bg-green-500 text-white px-8 py-3 rounded-2xl font-black text-sm cursor-pointer transition flex items-center gap-2"
            >
            ...
            <a
              href={getPdfUrl(pdfModal)}
              target="_blank"
              className="bg-blue-700 hover:bg-blue-600 text-white px-6 py-3 rounded-2xl font-black text-sm cursor-pointer transition flex items-center gap-2"
            >
              🔗 Yangi oynada ochish
            </a>
          </div>
