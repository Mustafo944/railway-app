"use client"
import { ShieldCheck, X, Save, Edit3, Trash2 } from 'lucide-react';
...
export default function AdminPanel({
  ...
  const toggleStation = (bekat, isEdit = false) => {
    if (isEdit) {
      ...
      if (current.includes(bekat)) {
        setEditStation(current.filter(b => b !== bekat));
      } else if (current.length < 3) {
        setEditStation([...current, bekat]);
      }
    } else {
      const current = Array.isArray(newWorkerStation) ? newWorkerStation : (newWorkerStation ? [newWorkerStation] : []);
      if (current.includes(bekat)) {
        setNewWorkerStation(current.filter(b => b !== bekat));
      } else if (current.length < 3) {
        setNewWorkerStation([...current, bekat]);
      }
    }
  };
    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex items-end sm:items-center justify-center p-0 sm:p-3">
      <div className="bg-white w-full sm:max-w-2xl h-[85vh] sm:h-[90vh] rounded-t-4xl sm:rounded-4xl shadow-2xl overflow-hidden flex flex-col animate-in slide-in-from-bottom sm:zoom-in-95 duration-300">
        ...
        <div className="p-4 border-b-4 border-orange-200 flex justify-between items-center bg-orange-50 sticky top-0">
          ...
          <button onClick={onClose} className="bg-white p-2 rounded-full text-orange-600 cursor-pointer shadow">
            <X size={20} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-3 sm:p-4 space-y-3">
          ...
          <input
            ...
            <select
              value={newWorkerRole || ''}
              onChange={e => { setNewWorkerRole(e.target.value); setNewWorkerStation(''); }}
              className="w-full p-2 sm:p-3 border-2 rounded-xl outline-none focus:border-orange-500 font-bold bg-white text-xs sm:text-sm"
            >
                      ...
                      return (
                        <button key={b} type="button" onClick={() => toggleStation(b)}
                          className={`px-2 py-1 rounded-lg text-[9px] font-black cursor-pointer transition-all ${
                            selected ? 'bg-orange-600 text-white' : 'bg-slate-100 text-slate-600'
                          }`}
                        >
                          {b}
                        </button>
            ...
            <button onClick={addWorker}
              className="w-full bg-orange-600 text-white p-2.5 sm:p-3 rounded-xl font-black cursor-pointer uppercase text-xs sm:text-sm hover:bg-orange-700 transition"
            >
              Qo'shish
            </button>
          </div>
          ...
          <div className="space-y-2">
            <p className="font-black text-[10px] sm:text-xs text-slate-500 px-1">
              📋 Ishchilar ro'yxati ({workersList.length})
            </p>

            {workersList.map((w) => (
              <div key={w.id}
                className={`bg-white border-2 rounded-xl sm:rounded-2xl p-3 sm:p-4 ${
                  isProtected(w.role) ? 'border-orange-200 bg-orange-50' : 'border-slate-100'
                }`}
              >
                {editingWorker?.id === w.id ? (
                  <div className="space-y-2">
                    <input
                      className="w-full p-2 border-2 rounded-xl text-xs sm:text-sm font-bold outline-none focus:border-orange-500"
                      placeholder="F.I.SH"
                      value={editName}
                    ...
                    <div className="flex gap-2">
                      <button onClick={saveEdit}
                        className="flex-1 bg-green-600 text-white p-2 rounded-xl font-black text-xs sm:text-sm cursor-pointer flex items-center justify-center gap-1"
                      >
                        <Save size={14}/> Saqlash
                      </button>
                      <button onClick={() => setEditingWorker(null)}
                        className="flex-1 bg-slate-200 p-2 rounded-xl font-black text-xs sm:text-sm cursor-pointer"
                      >
                        Bekor
                      </button>
                    </div>
                  ...
                  <div className="flex justify-between items-center">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1 flex-wrap">
                        ...
                        {w.station && (
                          <p className="text-[8px] sm:text-[10px] font-black text-purple-700">
                            📍 {stationDisplay(w.station)}
                          </p>
                        )}
                      </div>
<div className="flex items-center gap-1 mt-0.5 flex-wrap">
                      ...
                      {w.phone && (
                        <p className="text-[9px] sm:text-[10px] font-bold text-green-700 mt-0.5">
                          📞 {w.phone}
                        </p>
                      )}
                    </div>
                    <div className="flex gap-0.5 sm:gap-1">
                      <button onClick={() => handleEditClick(w)}
                        className="text-blue-600 p-1.5 sm:p-2 hover:bg-blue-50 rounded-full cursor-pointer"
                      >
                        <Edit3 size={16} />
                      </button>
                      {!isProtected(w.role) ? (
                        <button onClick={() => removeWorker(w)}
                          className="text-red-500 p-1.5 sm:p-2 hover:bg-red-50 rounded-full cursor-pointer"
                        >
                          <Trash2 size={16} />
                        </button>
                      ) : (
